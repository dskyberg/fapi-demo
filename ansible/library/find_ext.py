...
options:
    ...
    exclude_patterns:
        ...
        description:
            - One or more (shell or regex) patterns, which type is controlled by C(use_regex) option.
            - Files that match an exclude pattern will not be evaluated.
    contains:
        description:
    ...
    use_regex_exclude:
        ...
        choices: [ 'no', 'yes' ]
        description:
            - Same as 'use_regex', but for exclude_patterns.
notes:
    - For Windows targets, use the M(win_find) module instead.
'''
...
- name: Find all files equal except files ending in '.zip'
- find:
    paths: /var/log
...
def pExcludeFilter(f, exclude_patterns=None, use_regex_exclude=False):
    if exclude_patterns is None:
        return True

    if use_regex_exclude:
...
def pfilter(f, patterns=None, use_regex=False, exclude_patterns=None, use_regex_exclude=False):
    '''filter using glob patterns'''

    if patterns is None and exclude_patterns is None:
        return True
    ...
    if use_regex:
        for p in patterns:
            r = re.compile(p)
            if r.match(f):
                return pExcludeFilter(f, exclude_patterns, use_regex_exclude)
    else:
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            exclude_patterns=dict(type='list', default=[], aliases=['exclude_pattern']),
            ...
            use_regex_exclude=dict(type='bool', default='no'),
        ),
            ...
            for root, dirs, files in (sys.version_info < (2, 6, 0) and os.walk(npath)) or os.walk(npath, followlinks=params['follow']):
                ...
                for fsobj in (files + dirs):
                    ...
                    if params['file_type'] == 'any':
                        if pfilter(fsobj, params['patterns'], params['use_regex'], params['exclude_patterns'], params['use_regex_exclude']) and agefilter(st, now, age, params['age_stamp']):
                        ...
                        if pfilter(fsobj, params['patterns'], params['use_regex'], params['exclude_patterns'], params['use_regex_exclude']) and \
